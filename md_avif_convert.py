...
convert them to lossless AVIF using ffmpeg, and update the links in the Markdown files.
Backup the original image files to another directory before conversion.

Example usage:
    python md_avif_convert.py --source ./content --backup ./backup_images

Features:
 - Handles inline image links (e.g., ![alt](path/to/img.png "title")).
 - Skips URL references (http:// or https://).
 - Skips files already in .avif format.
...
Notes:
 ...
 - ffmpeg must support AV1 encoding (libaom-av1 or similar).
"""
...
logger = logging.getLogger("md_avif_convert")


def find_markdown_files(source_dir: Path) -> List[Path]:
    """Find all .md/.markdown files under source_dir."""
    results = []
...
def copy_to_backup(original: Path, source_root: Path, backup_root: Path) -> Path:
    """Copy the original file to backup_root preserving relative structure."""
    try:
...
def convert_image_to_avif(src: Path, dst: Path, ffmpeg_path: str = "ffmpeg") -> bool:
    """Convert image to lossless AVIF using ffmpeg."""
...
def replace_paths_in_markdown(md_text: str, replacements: List[Tuple[str, str]]) -> str:
    """Replace old image paths with new ones in Markdown text."""
...
def process_markdown_file(
    ...
    for img_ref, _ in imgs:
        ...
        try:
            ...
            continue
        avif_path = resolved.with_suffix(".avif")
        if not convert_image_to_avif(resolved, avif_path, ffmpeg_path=ffmpeg_path):
            logger.error(f"Conversion failed: {resolved}")
            continue
        try:
            new_rel = os.path.relpath(avif_path, start=md_path.parent)
        except Exception:
            new_rel = str(avif_path)
        new_rel = new_rel.replace("\\", "/")
        replacements.append((img_ref, new_rel))
    if replacements:
        new_text = replace_paths_in_markdown(text, replacements)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert images in Markdown to lossless AVIF and update links."
    )
    parser.add_argument(
        "--source",
        "-s",
